 ...
 */
extern void nx_boot_logo(U32 base, int x_resol, int y_resol, U32 pixelbyte);
...
static struct pwm_pad pwm_ch[] = {
	{ PAD_GET_GRP(PAD_GPIO_C), 15, NX_GPIO_PADFUNC_1 },
	{ PAD_GET_GRP(PAD_GPIO_C), 16, NX_GPIO_PADFUNC_1 },
	{ PAD_GET_GRP(PAD_GPIO_B),  5, NX_GPIO_PADFUNC_2 },
	{ PAD_GET_GRP(PAD_GPIO_B),  6, NX_GPIO_PADFUNC_2 }
};
...
static void init_display(int dev)
{
	...
	//--------------------------------------------------------------------------
	// Backlight Power IC Disable and LCD Off
	//--------------------------------------------------------------------------
	// Primary LCD
	//	if (dev == 0 && CFG_LCD_PRI_BACKLIGHT_GPIO == CTRUE)
	//	{
	//	NX_GPIO_SetOutputValue(PAD_GET_GRP(CFG_PIO_LCD_BLU_ENB), PAD_GET_BIT(CFG_PIO_LCD_BLU_ENB), CFALSE);	// Backlight Power IC Disable
	//	NX_GPIO_SetOutputValue(PAD_GET_GRP(CFG_PIO_LCD_PCI_ENB), PAD_GET_BIT(CFG_PIO_LCD_PCI_ENB), CFALSE);			// LCD Off
	//	}
	...
	//	if (dev == 0 && CFG_LCD_PRI_BACKLIGHT_GPIO == CTRUE)
	//	{
	//	NX_GPIO_SetOutputValue(PAD_GET_GRP(CFG_PIO_LCD_PCI_ENB), PAD_GET_BIT(CFG_PIO_LCD_PCI_ENB), CTRUE);			// LCD On
	//	}
...
dbg_reg_dpc(int dev)
{
	U32 DPCBASE = 0xC0003000 + (0x400*dev);
...
static void dbg_reg_mlc(int dev)
{
	U32 MLCBASE = 0xC0004000 + (0x400*dev);

	DBGOUT("MLCSCREENSIZE		      	=0x%08x\n", *(U32*)(MLCBASE + 0x004));	// MLC Screen Size Register
	DBGOUT("MLCBGCOLOR		      		=0x%08x\n", *(U32*)(MLCBASE + 0x008));	// MLC Background Color Register
	DBGOUT("MLCLEFTRIGHT	          	=0x%08x\n", *(U32*)(MLCBASE + 0x040));	// MLC RGB Layer Left Right Register 0/1
	DBGOUT("MLCTOPBOTTOM 	          	=0x%08x\n", *(U32*)(MLCBASE + 0x044));	// MLC RGB Layer Top Bottom Register 0/1
	DBGOUT("MLCINVALIDLEFTRIGHT0    	=0x%08x\n", *(U32*)(MLCBASE + 0x048));	// MLC RGB Invalid Area0 Left, Right Register 0/1
	DBGOUT("MLCINVALIDTOPBOTTOM0    	=0x%08x\n", *(U32*)(MLCBASE + 0x04C));	// MLC RGB Invalid Area0 Top, Bottom Register 0/1
	DBGOUT("MLCINVALIDLEFTRIGHT1    	=0x%08x\n", *(U32*)(MLCBASE + 0x050));	// MLC RGB Invalid Area1 Left, Right Register 0/1
	DBGOUT("MLCINVALIDTOPBOTTOM1    	=0x%08x\n", *(U32*)(MLCBASE + 0x054));	// MLC RGB Invalid Area1 Top, Bottom Register 0/1
	DBGOUT("MLCCONTROL	          		=0x%08x\n", *(U32*)(MLCBASE + 0x058));	// MLC RGB Layer Control Register 0/1
	DBGOUT("MLCHSTRIDE	          		=0x%08x\n", *(U32*)(MLCBASE + 0x05C));	// MLC RGB Layer Horizontal Stride Register 0/1
	DBGOUT("MLCVSTRIDE	          		=0x%08x\n", *(U32*)(MLCBASE + 0x060));	// MLC RGB Layer Vertical Stride Register 0/1
	DBGOUT("MLCTPCOLOR	          		=0x%08x\n", *(U32*)(MLCBASE + 0x064));	// MLC RGB Layer Transparency Color Register 0/1
	DBGOUT("MLCINVCOLOR		          	=0x%08x\n", *(U32*)(MLCBASE + 0x068));	// MLC RGB Layer Inversion Color Register 0/1
	DBGOUT("MLCADDRESS	          		=0x%08x\n", *(U32*)(MLCBASE + 0x06C));	// MLC RGB Layer Base Address Register 0/1
	DBGOUT("MLCGAMMACONT	            =0x%08x\n", *(U32*)(MLCBASE + 0x0EC));	// MLC Gama Control Register
	DBGOUT("MLCRGAMMATABLEWRITE		    =0x%08x\n", *(U32*)(MLCBASE + 0x0F0));	// MLC Red Gamma Table Write Register
	DBGOUT("MLCGGAMMATABLEWRITE		    =0x%08x\n", *(U32*)(MLCBASE + 0x0F4));	// MLC Green Gamma Table Write Register
	DBGOUT("MLCBGAMMATABLEWRITE		    =0x%08x\n", *(U32*)(MLCBASE + 0x0F8));	// MLC Blue Gamma Table Write Register
	DBGOUT("MLCCLKENB               	=0x%08x\n", *(U32*)(MLCBASE + 0x3C0));	//  MLC Clock Enable Register
	DBGOUT("MLCCONTROLT			      	=0x%08x\n", *(U32*)(MLCBASE + 0x000));	// MLC Top Control Register
}
#endif
