...
#define POLL_I2C	1	// 1 = polling, 0 = interrupt
...
#define DEBUG_I2C	0	// 0 = none, 1 = tracers, 2 = no reg caching
#define POLL_I2C	0	// 0 = polling, 1 = interrupt
#define	msleep(_n)		__udelay(_n*1000)

/*
 * used to set initial backlight value
 * Note that U-BOOT LCD comes up before I2C is ready,
 * so hang onto value and use when initializing the LFP100
 */
#define LFP100_WLED_BACKLIGHT	0x18		/* initial backlight	*/
u32	lfp100_wled_default = LFP100_WLED_BACKLIGHT;
...
static inline int is_board_lucy(void)
{
	int ret=0;
...
/* lfp100_wled_set()
 * try to update WLED register.  Ignore error, save new value
 * as I2C and LFP100 may not be initialized yet.
 */
void lfp100_wled_set(uint wled_value) {

	lfp100_wled_default = wled_value;	/* save new value */
	/* write reg if lfp100 found */
	if (!local_chip || !local_chip->have_lfp100)
		return;
	lfp100_write_reg(LFP100_WLED, wled_value);
}
...
static void lfp100_monitor_task(struct work_struct *work)
{
	...
	printk(KERN_INFO "%s: enter, busy=%d\n", __FUNCTION__, local_chip->busy);
...
int lfp100_set_vbus_draw(unsigned int mA)
{
	...
	if      (mA <=  100) iusb_value = LFP100_PPATH_IUSB_100MA;
	else if (mA <=  500) iusb_value = LFP100_PPATH_IUSB_500MA;
	else if (mA <= 1300) iusb_value = LFP100_PPATH_IUSB_1300MA;
	else if (mA <= 1800) iusb_value = LFP100_PPATH_IUSB_1300MA;
	else return -ENOTSUPP;

	lfp100_ppath_reg = lfp100_read_reg(LFP100_PPATH);
	lfp100_ppath_reg &= ~(LFP100_PPATH_IUSB_MASK);
	lfp100_ppath_reg |= iusb_value;
	lfp100_write_reg(LFP100_PPATH, lfp100_ppath_reg);

	printk(KERN_INFO "%s vbus draw is %d mA\n", __FUNCTION__, mA);
	return 0;
}
...
static void lfp100_show_power_source()
{
	//usb_powered = 1;
	u8 lfp100_status1_reg;
	u8 lfp100_int2_reg;
	...
	//if(lfp100_int2_reg & LFP100_INT2_BP)
		//printk(KERN_INFO "Push button pressed \n");
		
	//printk(KERN_INFO "usb powered = %u\n", local_chip->usb_powered);
		
	printk(KERN_INFO "%s: LFP100 reports power source is ", __FUNCTION__);	
	...
	case LFP100_STATUS1_SOURCE_AC:
		...
		if(!(lfp100_int2_reg & LFP100_INT2_BP))
		{
			local_chip->usb_powered = 0;
			//printk(KERN_INFO "not usb powered \n");
		}
		break;
	case LFP100_STATUS1_SOURCE_USB:
		...
		break;
	case LFP100_STATUS1_SOURCE_BAT:
		printk(KERN_INFO "Battery \n");
		break;
	case LFP100_STATUS1_SOURCE_UNDEF:
		printk(KERN_INFO "Undefined \n");
		break;
	}
	printk(KERN_INFO " (STATUS1:0x%2.2X).\n", lfp100_status1_reg);
	
	//return usb_powered;
}

unsigned int lfp100_is_usb_powered()
{
	if(local_chip->usb_powered)
		return 1;
	else return 0;
}
EXPORT_SYMBOL(lfp100_is_usb_powered);

static void lfp100_setup_backlight()
{
	/* set backlight level */
	lfp100_write_reg(LFP100_WLED, lfp100_wled_default);
...
void lfp100_backlight_fadeout()
{
	int i;
	unsigned int value = lfp100_read_reg(LFP100_WLED);
	//printf("backlight initial value = %u \n", value);
	//NX_GPIO_SetOutputValue(0, 30, CTRUE); // Pull CNTRL high
	
	for (i = value - 1; i >= 0; i--)
	{
		lfp100_write_reg(LFP100_WLED, i);
		msleep(25);
	}
}
EXPORT_SYMBOL(lfp100_backlight_fadeout);
...
/*
 * Try setting CPU from 500 MHz up to 800 MHz, assuming system has LFP100
 */
#define PLL0		0
#define	PLL0_PDIV	3
#define PLL0_SDIV	1

#define PLL0_MDIV_MIN_666	( 666 / 2)	/* CPU MHZ divided by 2 */
#define PLL0_MDIV_MAX_666	( 666 / 2)	/* CPU MHZ divided by 2 */

#define PLL0_MDIV_MIN_700	( 700 / 2)	/* CPU MHZ divided by 2 */
#define PLL0_MDIV_MAX_700	( 700 / 2)	/* CPU MHZ divided by 2 */

#define PLL0_MDIV_MIN_800	( 800 / 2)	/* CPU MHZ divided by 2 */
#define PLL0_MDIV_MAX_800	( 800 / 2)	/* CPU MHZ divided by 2 */

#define SETTLING_IN_MS	100

// FIXME: sesters  Tune NAND timings for Rio
#define NAND_BUSWIDTH_RIO	8	// =8 Fixed
#define NAND_TACS_RIO		0xF
#define NAND_TCOS_RIO		1
#define NAND_TACC_RIO		5
#define NAND_TSACC_RIO		6
#define NAND_TCOH_RIO		0
#define NAND_TCAH_RIO		0xF
#define NAND_WAITMODE_RIO	1	// =1 Fixed
#define NAND_BURSTREAD_RIO	0	// =0 Fixed
#define NAND_BURSTWRITE_RIO	0	// =0 Fixed
...
int init_lfp100(void)
{
	...
	int ret = 0;
	u8 lfp100_ppath_reg;
	u16 pll0_mdiv_min = PLL0_MDIV_MIN_800;
	u16 pll0_mdiv_max = PLL0_MDIV_MAX_800;
	...
	//		     LFP100_A_CONTROL	  can cache, LFP100 Read-Only
	...
	//		     LFP100_CHGCONFIG0	  can cache, LFP100 Read-Only
	...
	for (i = 0; i <= priv->maximum_register; i++) {
		if(priv->reg_properties[i] != REG_IS_VOLATILE)
		{
			...
			/* uncomment for debugging boot */
			// printk(KERN_DEBUG "%s: reg %02X: %02X\n", __FUNCTION__, i, ret);
		}
	}
	priv->have_lfp100 = 1;		/* read registers */
	priv->usb_powered = 1;      
	
	lfp100_setup_backlight();
	
	/*Setup interrupt registers to listen for Push Button */
	lfp100_write_reg(LFP100_MASK2, ~(LFP100_MASK2_PBM));
	
	/* show power source */
	lfp100_show_power_source();
	//printk(KERN_INFO "usb powered = %u\n", priv->usb_powered);

//FIXME: don't limit current on Rio builds during development
#if (defined(CONFIG_MACH_NXP3200_RIO_DTK))
#warning "U-Boot does not limit current to 100MA on Rio"
#endif
	...
	if ((lfp100_read_reg(LFP100_CHIPID) & LFP100_CHIPID_CHIP_MASK)
		== LFP100_CHIPID_CHIP_LFP300) {
		printk(KERN_INFO "%s.%d:%s  CHGCONFIG1 (0x%2.2X)",
			__FILE__, __LINE__, __FUNCTION__, lfp100_read_reg(LFP100_CHGCONFIG1));
		lfp100_write_reg(LFP100_CHGCONFIG1,
			lfp100_read_reg(LFP100_CHGCONFIG1) | LFP100_CHGCONFIG1_CHG_EN);
		printk(KERN_INFO "  Enable Charger => (0x%2.2X)\n",
			lfp100_read_reg(LFP100_CHGCONFIG1));
	}
	...
	case LF2000_BOARD_RIO_FEP_800_327P666:
		/* 800 MHz CPU */
		pll0_mdiv_min = PLL0_MDIV_MIN_800;
		pll0_mdiv_max = PLL0_MDIV_MAX_800;
	
		lfp100_write_reg(LFP100_DCDC1_PW, LFP100_DCDC1_PW_1P3_VOLTS);
		printk(KERN_INFO "%s: LFP100 DCDC1 set to 0x%X\n",
			__FUNCTION__, lfp100_read_reg(LFP100_DCDC1_PW));
		mdelay(SETTLING_IN_MS);// Give power time to settle to new level

		/* adjust NAND bus */
		NX_MCUS_SetStaticBUSConfig(NX_MCUS_SBUSID_NAND, NAND_BUSWIDTH_RIO,  NAND_TACS_RIO,
					   NAND_TCAH_RIO,       NAND_TCOS_RIO,      NAND_TCOH_RIO,
					   NAND_TACC_RIO,       NAND_TSACC_RIO,     NAND_WAITMODE_RIO,
					   NAND_BURSTREAD_RIO,  NAND_BURSTWRITE_RIO);

		/* adjust HCLK divisor before changing */
		NX_CLKPWR_SetClockCPU(PLL0, CFG_SYS_CPU_COREDIV, CFG_SYS_CPU_BUSDIV_800); // CPU PLL0 / 1 / 4 / 2

		/* keep loop in case we need to retune trying different CPU speeds */
		for (i=pll0_mdiv_min; i <= pll0_mdiv_max; i+=5) {
			printk(KERN_INFO "%s.%d  i:%d\n", __FUNCTION__, __LINE__, i);
			while (CFALSE == NX_CLKPWR_IsPLLStable()) { ; } // stable PLL

			NX_CLKPWR_SetPLLPMS (PLL0, PLL0_PDIV, i, PLL0_SDIV);
	...
	case LF2000_BOARD_RIO_EP_700_400:
		/* 700 MHz CPU */
		pll0_mdiv_min = PLL0_MDIV_MIN_700;
		pll0_mdiv_max = PLL0_MDIV_MAX_700;
		...
		/* adjust HCLK divisor before changing */
		NX_CLKPWR_SetClockCPU(PLL0, CFG_SYS_CPU_COREDIV, CFG_SYS_CPU_BUSDIV_700); // CPU PLL0 / 1 / 4 / 2
	...
	case LF2000_BOARD_RIO_EP_666_333:
		/* 666 MHz CPU */
		pll0_mdiv_min = PLL0_MDIV_MIN_666;
		pll0_mdiv_max = PLL0_MDIV_MAX_666;
...
/* handle power_button, shutdown when power button is released */
...
void lfp100_standby(void)
{
	...
	/* Changed so power goes off when the power button is released for
	 * the second time.  Before this change, the unit shut off if a user
	 * pressed the power button for too long after entering tune-up
	 * mode before releasing it.
	 */
int power_button_pressed  = 0;	/* 1=power button pressed again after release */
int power_button_released = 0;	/* 1=power button released */

void lfp100_monitor_power_button(void)
{
	if (lfp100_get_power_button()) {
		/* button pressed */
		if (power_button_released)
			power_button_pressed = 1;
	} else { /* button released */
		power_button_released = 1;
		if (power_button_pressed) { /* release after press -> standby */
			lfp100_standby();
		}
	}
}
...
static int lfp100_chip_probe(struct platform_device *pdev)
{
	...
	lfp100_write_reg(LFP100_MASK3, ~(LFP100_MASK3_HP | LFP100_MASK3_ABUSY));
