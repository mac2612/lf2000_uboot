				...
				const size_t length)
{
	...
	size_t num_relevant;
	size_t bytes_remaining_in_block;

	while (len_excl_bad < length) {
		/* usuallly all remaining bytes in the block are relevant */
		bytes_remaining_in_block = nand->erasesize - 
					    (offset & (nand->erasesize - 1));
		num_relevant = bytes_remaining_in_block;

		/* but if we don't need that many, only those we need are
		 * relevant
		 */
		if (remaining_len < num_relevant)
			num_relevant = remaining_len;
		block_len = num_relevant;
		...
		{ 
			...
			len_incl_bad  += bytes_remaining_in_block;
			offset        += bytes_remaining_in_block;
			//remaining_len -= block_len;
		}
		if (offset >= nand->size)
			break;
			...
			u_char *buffer)
{
	...
	while (left_to_write > 0) {
		...
		if (nand_block_isbad (nand, offset & ~(nand->erasesize - 1))) {
			printf ("Skip bad block 0x%08x%08x\n",
				(uint)((offset & ~(nand->erasesize - 1))>>32),
				(uint)((offset & ~(nand->erasesize - 1)) &-1));
