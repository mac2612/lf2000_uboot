...
void proto_init_preNOR(void);
int nxp3200_serial_init(void);
void proto_init_postNOR(void);

void cpu_mach_init(void)
{
	bus_init();		// setup buses, including NOR timing
	proto_init_preNOR();	// setup not relying on NOR mfgdata
	init_baudrate();	// init serial so we can printf();
	nxp3200_serial_init();	// serial communications setup
	init_gpio_pad();	// setup so we can read NOR
	proto_init_postNOR();	// setup after NOR data is available
	pll_init();
}
...
void proto_init_preNOR(void)
{
	...
	for (idx = 0; NX_MLC_GetNumberOfModule() > idx; idx++) {
		NX_MLC_SetBaseAddress(idx, (U32)IO_ADDRESS(NX_MLC_GetPhysicalAddress(idx)));
	...
	/* ECID reading for CPU identification. */
	NX_ECID_Initialize();
...
void proto_init_postNOR(void)
{
	...
	/* SPI For Display setup */
	NX_SSPSPI_Initialize();
	NX_SSPSPI_SetBaseAddress(get_lf2000_board_info()->spi_lcd,
		(U32)IO_ADDRESS(NX_SSPSPI_GetPhysicalAddress(get_lf2000_board_info()->spi_lcd)));
	NX_SSPSPI_OpenModule(get_lf2000_board_info()->spi_lcd);
}
...
void bus_init(void)
{
	...
	/*
	 * NAND Bus config
	 * set all NAND chip banks to slower access speed
	 */


	{
		struct NX_MCUS_RegisterSet * pRegister =
...
#if 0	/* TODO: FIXME: move definitions for static bus 0 to cfg_sys.h? */
	/* TODO: FIXME: delete configurations of static buses 1-7.      */
	/* TODO: FIXME: after a non-NOR boot, configure static bus 13 for NOR */
	NX_MCUS_SetStaticBUSConfig(0,8,0,0,0,0,13,13,1,0,0);
#else
...
void pll_init(void)
{
	pre_fclk = cpu_get_clock_hz(CLOCK_FCLK);
	pre_mclk = cpu_get_clock_hz(CLOCK_MCLK);
...
u_int cpu_get_clock_hz(int clk)
{
	...
	switch (clk) {
	case CLOCK_PLL0:	clock_hz =	pll[0];	break;
	case CLOCK_PLL1:	clock_hz =	pll[1];	break;
	case CLOCK_FCLK:	clock_hz =	fclk;	break;
	case CLOCK_MCLK:	clock_hz =	mclk;	break;
	case CLOCK_BCLK:	clock_hz =	bclk;	break;
	case CLOCK_PCLK:	clock_hz =	pclk;	break;
	default:
		DBGOUT("error: unknown clock [%d], (0~5)\n", clk);
		break;
	}
	return clock_hz;
}
