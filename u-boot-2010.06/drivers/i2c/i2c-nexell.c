...
struct i2c_adapter *i2c_get_adapter(int id) {
	if ( (i2c == NULL) || (id < 0) || (id >= NUM_I2C_PORTS))
		return NULL;
	return ((struct i2c_adapter *)&i2c[id]);
}
...
static int	i2c_init_func(struct i2c_port *i2c, int port )
{
	int ret = 0;
	u_int pclk;

	/* set i2c hardware */
	i2c->hw.port	 = port;	          /*plat->port;*/
	switch (port) {
	case 0:
		i2c->hw.io_scl	 = CFG_PIO_I2C0_SCL_;
		i2c->hw.io_sda	 = CFG_PIO_I2C0_SDA_;
		i2c->hw.clksrc	 = CFG_I2C0_CLOCK_SOURCE;

		/* adjust clkscale for Rio, if needed */
		i2c->hw.clkscale = CFG_I2C0_CLOCK_SCALER_GENERIC;
		...
		i2c->hw.cntmax 	 = CFG_I2C0_CLOCK_DELAY;
		i2c->hw.polling	 = 1;	

		i2c->polling	 = 1;
		i2c->gpio	 = (i2c->hw.clksrc == I2C_SOURCE_GPIO) ? 1 : 0;
		i2c->nostop	 = false;
		break;
	case 1:
		i2c->hw.io_scl	 = CFG_PIO_I2C1_SCL_;	  /*plat->io_scl;*/
		i2c->hw.io_sda	 = CFG_PIO_I2C1_SDA_;	  /*plat->io_sda;*/
		i2c->hw.clksrc	 = CFG_I2C1_CLOCK_SOURCE; /*plat->clksrc;*/
		i2c->hw.clkscale = CFG_I2C1_CLOCK_SCALER; /*plat->clkscale;*/
		i2c->hw.cntmax 	 = CFG_I2C1_CLOCK_DELAY;  /*plat->cntmax;*/
		i2c->hw.polling	 = 1;			  /*plat->polling;*/

		i2c->polling	 = 1;			  /*plat->polling;*/
		i2c->gpio	 = (i2c->hw.clksrc == I2C_SOURCE_GPIO) ? 1 : 0;
		i2c->nostop	 = false;
		break;
	default:
//		printf("i2c_init_func(): unexpected port: %d\n", port);
		return -1;
	}


	if (i2c->gpio) {
		printf("%s.%d: gpio mode\n", I2C_DEV_NAME, i2c->hw.port);
		return ret;
	}

	/* for I2C bus */
	NX_I2C_SetBaseAddress(i2c->hw.port, 
			(U32)IO_ADDRESS(NX_I2C_GetPhysicalAddress(i2c->hw.port)));

	/* 0=PLL0, 1=PLL1, 2=FCLK, 3=MCLK, 4=BCLK, 5=PCLK */
	pclk = cpu_get_clock_hz(5);
	printk("%s.%d: %8d hz [pclk=%d, src=%3d, scale=%2d][%s] \n",
		I2C_DEV_NAME, i2c->hw.port, 
		(pclk/i2c->hw.clksrc)/(i2c->hw.clkscale),
		pclk, i2c->hw.clksrc, i2c->hw.clkscale, 
		i2c->polling?"polling":"irq");
...
//printf("i2c_set_io_func: func 0x%x, pio 0x%x, scl %d, sda %d\n", 
//	func, pio, scl, sda);
	/*  set scl pad */
...
static int i2c_transfer_hw(struct i2c_port *i2c, struct i2c_msg *msg, int num)
{
	...
	mutex_lock(&i2c->lock);
#endif
//printf("i2c_transfer_hw(msg 0x%x, num %d)\n", 
//	(unsigned int)msg, num);
	...
	/* transfer */
//printf("i2c_transfer_hw#1\n");
	if (! i2c_trans_start(i2c, msg))
		goto err_i2c;

//printf("i2c_transfer_hw#2\n");
	if (! i2c_trans_data(i2c, msg))
		goto err_i2c;

//printf("i2c_transfer_hw#3\n");
	ret = msg->len;

err_i2c:

//printf("i2c_transfer_hw#4: ret %d, msg->len %d\n", ret, msg->len);
	if (ret != msg->len)
		msg->flags &= ~I2C_M_NOSTART;

//printf("i2c_transfer_hw#5\n");
	i2c_trans_stop(i2c, msg);

//printf("i2c_transfer_hw#6\n");
	if (ret == msg->len && ! i2c->nostop)
		i2c_wait_busy(i2c);

	if (! i2c->nostop)
		i2c_set_clock(i2c, false);
//printf("i2c_transfer_hw#7; ret %d\n", ret);
...
//printf("nx_i2c_algo_xfer#1; i2c->gpio %d\n", i2c->gpio);
	if (i2c->gpio)
...
//printf("nx_i2c_algo_xfer#2; num %d; tmsg->len %d\n", num, tmsg->len);
		len = tmsg->len;

		/* transfer */
		for (i = adap->retries; i > 0; i--) {
//printf("nx_i2c_algo_xfer#3; i %d\n", i);
			ret = transfer_i2c(i2c, tmsg, num);
//printf("nx_i2c_algo_xfer#4; ret %d\n", ret);
			if (ret == len) {
				break;
			}
			ERROUT("i2c:%d (try:%d)\n", i2c->hw.port, adap->retries-i+1);
		}
//printf("nx_i2c_algo_xfer#5; ret %d, len %d\n", ret, len);
...
//printf("nx_i2c_algo_xfer#6; ret %d\n", ret );
	return ret;
}
...
int nx_i2c_init(void)
{
	int port;
	int ret = 0;

//printf("nx_i2c_init(): i2c 0x%x\n", (unsigned int)i2c );
		/* if we haven't already initialized, do so now */
	if (i2c != NULL ) {
		printf("%s:%s.%d  (i2c:0x%p) already initialized\n",
			__FILE__, __func__, __LINE__, i2c);
	} else {
		/* allocate i2c_port data */
		i2c = kzalloc(NUM_I2C_PORTS * sizeof(struct i2c_port), GFP_KERNEL);
		if (!i2c) {
			printk(KERN_ERR "fail, to allocate i2c driver info ...\n");
			return -ENOMEM;
		}
//printf("  &i2c[0] 0x%x, &i2c[1] 0x%x\n",
//	(unsigned int)&i2c[0], (unsigned int)&i2c[1]);

		for (port = 0; port < NUM_I2C_PORTS; ++port)
		{
			/* init i2c data struct */
			ret = i2c_init_func(&i2c[port], port);
			if (ret)
				goto err_mem;

			/*	init i2c adapter */
			strncpy(i2c[port].adap.name, I2C_DEV_NAME, 
				sizeof(I2C_DEV_NAME));

			i2c[port].adap.nr 	 = i2c[port].hw.port;
			i2c[port].adap.algo 	 = &nx_i2c_algo;
			i2c[port].adap.algo_data = &i2c[port];
			i2c[port].adap.retries 	 = I2C_RETRY_COUNT;

//printf("nx_i2c_init: &i2c[port].adap.algo 0x%x, master_xfer 0x%x, (0x%x)\n",
//	(unsigned int)&i2c[port].adap.algo, 
//	(unsigned int)i2c[port].adap.algo->master_xfer,
//	(unsigned int)nx_i2c_algo_xfer);

			/* set i2c io */
			i2c_set_io_func(&i2c[port]);
		}
	}
//printf("nx_i2c_init() returns %d: i2c 0x%x\n", ret, (unsigned int)i2c );
	return ret;
...
/* copied from linux/drivers/i2c/i2c-core.c, then modified for u-boot */
/**
...
int i2c_transfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	 ...
	 */
//printf("        i2c_transfer(0x%x, 0x%x, %d)\n",
//	(unsigned int)adap, (unsigned int)msgs, num);
	if (adap->algo->master_xfer) {
//printf("        i2c_transfer#1; adap->algo->master_xfer: 0x%x\n",
//	(unsigned int)adap->algo->master_xfer);
...
//printf("        i2c_transfer#2; \n");
			ret = adap->algo->master_xfer(adap, msgs, num);
//printf("        i2c_transfer#3; ret %d\n", ret);
			if (ret != -EAGAIN)
				break;
#ifndef U_BOOT
...
#endif
//printf("        i2c_transfer#4\n");

		return ret;
	} else {
//		printf( "I2C level transfers not supported\n");
		return -EOPNOTSUPP;
	}
}
