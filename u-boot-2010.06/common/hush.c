...
# endif
...
#ifdef OUTPUT_TIMING
	output_timing("run_pipe_real#1");
	printf("  pi->num_progs %d\n", pi->num_progs);
#endif	/* 5dec11 */
	nextin = 0;
#ifndef __U_BOOT__
	pi->pgrp = -1;
	...
	if (pi->num_progs == 1 && child->group && child->subshell == 0) {
		...
		int rcode;
		debug_printf("non-subshell grouping\n");
		output_timing("run_pipe_real->run_list_real()");
		rcode = run_list_real(child->group);
#endif
		return rcode;
	} else if (pi->num_progs == 1 && pi->progs[0].argv != NULL) {
		output_timing("run_pipe_real#2");
		for (i=0; is_assignment(child->argv[i]); i++) { /* nothing */ }
		if (i!=0 && child->argv[i]==NULL) {
			output_timing("run_pipe_real#2a");
		...
		output_timing("run_pipe_real#2b");
		if (child->sp) {
			char * str = NULL;

			str = make_string((child->argv + i));
#ifdef OUTPUT_TIMING
			printf("run_pipe_real#2c: str '%s'\n", str);
#endif	/* 5dec11 */
	    ...
	    extern int do_bootd (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
				...
				output_timing("run_pipe_real#2e");
...
#ifdef OUTPUT_TIMING
				printf("run_pipe_real#2f: 0x%x", 
					(unsigned int)cmdtp->cmd);
#endif	/* 5dec11 */
				output_timing("");
				rcode = (cmdtp->cmd)(cmdtp, flag,child->argc-i,
							&child->argv[i]);
				if ( !cmdtp->repeatable )
					flag_repeat = 0;


				output_timing("run_pipe_real#2f2");
#ifdef OUTPUT_TIMING
				printf("  rcode %d\n", rcode);
#endif	/* 5dec11 */
#endif
...
#endif
		output_timing("run_list_real->run_pipe_real()");
		rcode = run_pipe_real(pi);
#ifdef OUTPUT_TIMING
		printf("run_list_real:run_pipe_real returned %d\n", rcode);
#endif	/* 5dec11 */
...
#ifdef OUTPUT_TIMING
	printf("run_list_real#3; rcode %d\n", rcode);
#endif	/* 5dec11 */
	return rcode;
}
...
int parse_stream_outer(struct in_str *inp, int flag)
{
	...
	int code = 0;
#endif
	output_timing("parse_stream_outer#1");
	do {
		ctx.type = flag;
		initialize_context(&ctx);
		update_ifs_map();
...
#ifdef OUTPUT_TIMING
		printf("parse_stream_outer#2; rcode %d\n", rcode);
#endif	/* 5dec11 */
#ifdef __U_BOOT__
		...
		if (rcode != 1 && ctx.old_flag == 0) {
			output_timing("parse_stream_outer#3");
			done_word(&temp, &ctx);
			done_pipe(&ctx,PIPE_SEQ);
#ifndef __U_BOOT__
			run_list(ctx.list_head);
			...
			output_timing("parse_stream_outer#4");
			if (ctx.old_flag != 0) {
				free(ctx.stack);
				b_reset(&temp);
			}
		...
		b_free(&temp);
		output_timing("parse_stream_outer#5");
	} while (rcode != -1 && !(flag & FLAG_EXIT_FROM_LOOP));   /* loop on syntax errors, return on EOF */
#ifdef OUTPUT_TIMING
	printf("parse_stream_outer#6; code %d\n", code);
#endif	/* 5dec11 */
#ifndef __U_BOOT__
	return 0;
#else
