...
#ifdef OUTPUT_TIMING	
	printf("bootm_load_os#1; comp %d; tcount %d\n", 
		comp, NX_TIMER_GetTimerCounter(0));
#endif	
	switch (comp) {
	case IH_COMP_NONE:
		if (load == blob_start) {
...
#ifdef OUTPUT_TIMING
			printf("\nbootm_load_os#1a; start 0x%x, len %d; tcount %d\n", 
				(unsigned int)image_start, (unsigned int)image_len,
				NX_TIMER_GetTimerCounter(0));
#endif
			memmove_wd ((void *)load, (void *)image_start,
					image_len, CHUNKSZ);
			output_timing("bootm_load_os#1b");
		}
...
#ifdef OUTPUT_TIMING
	printf("bootm_load_os#2; comp %d; tcount %d\n", 
		comp, NX_TIMER_GetTimerCounter(0));
#endif
	puts ("OK\n");
...
#if 1	/* 6apr12  CBF stuff -- here temporarily just for experiment */
...
/* Structure is as follows:
  ...
  not here >>	data -- kernel image + root fs if any )
...
#define CBF_MAGIC	0x9abcdef0
u32 cbf_checksum (u8 *buf, u32 len, u32 incoming);


static int cbf_header_follows_image( image_header_t * hdr ) 
{
	struct cbf_hdr * ph; 
	int		 expected_size;
	u32		 crc;

	expected_size = sizeof(image_header_t) + image_get_data_size(hdr);
	ph	      = (struct cbf_hdr *)(((char *)hdr) + expected_size);

	if (ph->magic != 0x9abcdef0) {
		printf ("cbf_validate_header: magic wrong: 0x%x\n", ph->magic);
		return 0;
	}
	if (ph->version != 1) {
		printf ("cbf_validate_header: wrong version: %d\n", 
			 ph->version);
		return 0;
	}
	if (ph->num_bytes != expected_size) {
		printf ("cbf_validate_header: wrong size: %d, expected %d\n", 
			 ph->num_bytes, expected_size);
		return 0;
	}

	/* validate the CBF header's crc */
	crc = cbf_checksum ((u8 *)ph, 
			    (u32)((char *)&ph->crc_header - (char *)ph), 0);
	if (crc != ph->crc_header) {
		printf ("cbf_validate_header: checksum wrong.  "
			"Calc'd 0x%x, expected 0x%x\n", crc, ph->crc_header);
		return 0;
	}
	return 1;
}

static u32 cbf_crc_for_image( image_header_t * hdr)
{
	struct cbf_uImage * p; 
	int		    expected_size;

	expected_size = sizeof(image_header_t) + image_get_data_size(hdr);
	p	      = (struct cbf_uImage *)(((char *)hdr) + expected_size);
	return p->crc_data;
}
...
*/ image_header_t *image_get_kernel (ulong img_addr, int verify)
{
	...
	output_timing("image_get_kernel#3");

	if (verify) {
#if 1	/* 6apr12 */
	/* Check if uImage is followed by a valid CBF header.  If it is,
	 * assume that the 32-bit value following the end of the CBF header
	 * is the CBF-crc of uImage.  Since it's quicker to calculate a
	 * CBF-crc than to check the uImage CRC, do the CBF calculation.
	 */
		if ( cbf_header_follows_image( hdr )) {
			u32 stored_cbfcrc;
			u32 calcd_cbfcrc;

			puts ("   Verifying CBF crc ... ");
			stored_cbfcrc = cbf_crc_for_image(hdr);
			calcd_cbfcrc  = cbf_checksum( (u8 *)hdr, 
		      			(u32)image_get_data_size(hdr)
						+ sizeof(image_header_t), 0);
			if (stored_cbfcrc != calcd_cbfcrc) {
				printf ("Bad Data CRC:"
					" stored 0x%x, calc'd 0x%x\n",
					stored_cbfcrc, calcd_cbfcrc);
				show_boot_progress (-3);
				return NULL;
			}
		}
	...
	output_timing("image_get_kernel#4");

#if 0	/* for comparison of CBF verification time */
	/* with u-boot crc calculation time        */
extern u32 cbf_checksum (u8 *buf, u32 len, u32 incoming);
	cbf_checksum( (u8 *)image_get_data (hdr), 
		      (u32)image_get_data_size (hdr), 0);
#endif
	output_timing("image_get_kernel#4a (CBF)");
